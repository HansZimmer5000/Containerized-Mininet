...
function ubuntuOvs {
    ...
    if $pkginst openvswitch-testcontroller_$OVS_RELEASE*.deb 2>/dev/null; then
        echo "Ignoring error installing openvswitch-testcontroller"
    fi
    ...
    if sudo service openvswitch-testcontroller stop 2>/dev/null; then
        echo "Stopped running controller"
    fi
    if [ -e /etc/init.d/openvswitch-testcontroller ]; then
...
function ovs {
    ...
    if [ "$DIST" = "Fedora" -o "$DIST" = "RedHatEnterpriseServer" ]; then
        $install openvswitch openvswitch-testcontroller
        return
    fi
